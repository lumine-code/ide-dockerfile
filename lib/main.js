...
const setting = (key, scope) =>
  scope
    ? lumine.config.get(`ide-dockerfile.${key}`, { scope: [scope] })
    : lumine.config.get(`ide-dockerfile.${key}`);
...
const languageServerSettings = () => ({
  diagnostics: Object.fromEntries(
    diagnosticKeys.map((key) => [
      key,
      setting("features.diagnostics", "source.dockerfile")
...
module.exports = {
  provideBackgroundTips() {
    return {
      packageName: "ide-dockerfile",
      tips: [
        "{{packageName}} provides diagnostics, completion, navigation, formatting, and refactoring whenever a Dockerfile is open.",
      ],
    };
  },
